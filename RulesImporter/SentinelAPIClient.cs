...
namespace RulesImporter
{
    public class SentinelLAWClient
    {
        ...
        private void generateRequestBaseURL(string __ruleID)
        {
            RequestBaseUrl =
        ...
        public async Task<HttpResponseMessage> WriteAnalyticsRule(SentinelScheduledAlerts.AlertRule alertRule, string _ruleID)
        {
            ...
            authSignature = result.AccessToken;

            var dateTimeNow = DateTime.UtcNow.ToString("r");
            generateRequestBaseURL(_ruleID);
